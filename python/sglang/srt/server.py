...
def launch_engine(
    ...
    if server_args.dp_size == 1:
        ...
        reader, writer = mp.Pipe(duplex=False)
        scheduler_startup_chans = [reader]
        proc = mp.Process(
    ...
    scheduler_infos = []
    for i in range(len(scheduler_startup_chans)):
        try:
            data = scheduler_startup_chans[i].get()
...
def launch_server(
    ...
    """
    launch_engine(server_args=server_args, tokenizer_init_chan=tokenizer_init_chan)
